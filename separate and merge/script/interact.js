...
function isSequence(txt_inhalt){
    /* Inhalt as DNA sequence or aa sequence, none-flag */
    const inhaltregex = /[^ACGTURYKMSWBDHVN\-]|[^A-Z\*\-]/;
    let seqs = [];
    if(txt_inhalt != ""){
        let lines = txt_inhalt.split('\n');
        for(let i = 0; i < lines.length; i++){
            let line = lines[i];
            /*If there are letters other than the fasta-letter, will not be accepted*/
            if(line.match(inhaltregex, "i")){
                continue;
            }
            else{
                seqs.push(line);
